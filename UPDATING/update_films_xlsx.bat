...
rd /s/q ..\collection\%COLLECTION%_TMP2 2>&1
ECHO.

ECHO ---- Generating .xlsx file for collection at temp folder
cd ..\%SOURCE%
%PAUTHOR% /source cxml %COLLECTION%.cxml /target excel ..\collection\%COLLECTION%_TMP2\%COLLECTION%.xlsx && (goto OK) || (goto Fail)
...
move /Y ..\collection\%COLLECTION%_TMP2\%COLLECTION%_images ..\collection 2>&1
ECHO.

ECHO --- Copying XLSX file from temp folder, overwriting old one
move /Y ..\collection\%COLLECTION%_TMP2\%COLLECTION%.xlsx ..\collection 2>&1
ECHO.
...
rd ..\collection\%COLLECTION%_TMP2 2>&1
ECHO.
